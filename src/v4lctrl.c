...
int try_ged_shm_id(char *device){
	...
	//printf("\nuse shm keys 0x%x 0x%x....\n", data_key, semaphores_key);	
	
	if ((id = semget(semaphores_key, SEMAPHORES_COUNT, 0666 )) < 0) {
        //printf("error: shmget 1\n");
        return id;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (try_ged_shm_id(device) < 0 ){
        printf("try direct access to %s\n",device);    
        fd = open(device,O_RDONLY);
        printf("device %s open for read",device);
    }else{
        printf("%s is controled by other application, try communicate via shm\n",device);
    }
